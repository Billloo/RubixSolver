...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "author", content: "RubixSolver" },
      ...
      <div className="flex min-h-screen flex-col bg-background text-foreground">
        <header className="sticky top-0 z-20 border-b border-border/70 bg-background/85 backdrop-blur">
          <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
            <Link to="/" className="font-display text-lg font-semibold tracking-tight">Rubix<span className="text-primary">Solver</span></Link>
            <nav className="flex items-center gap-1 text-sm">
              {["2x2", "3x3"].map((slug) => (
                <Link key={slug} to="/solver/$puzzle" params={{ puzzle: slug }} activeProps={{ className: "text-primary" }} className="rounded-lg px-3 py-1.5 text-muted-foreground transition hover:text-foreground">{slug}</Link>
              ))}
              <Link to="/notation" activeProps={{ className: "text-primary" }} className="rounded-lg px-3 py-1.5 text-muted-foreground transition hover:text-foreground">Notation</Link>
            </nav>
          </div>
        </header>
