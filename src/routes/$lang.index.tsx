...
function LocalizedHome() {
  ...
  return <div lang={lang} className="min-h-screen">
    <main>
      <section className="hero-glow"><div className="mx-auto grid w-full max-w-6xl items-center gap-10 px-4 py-12 sm:gap-12 sm:px-6 sm:py-20 lg:grid-cols-[1.05fr_1fr] lg:py-28"><div><p className="text-sm font-medium uppercase tracking-[0.2em] text-primary">{c.learn}</p><h1 className="mt-5 max-w-4xl text-4xl font-semibold leading-tight sm:text-5xl lg:text-6xl">{c.homeTitle}</h1><p className="mt-5 max-w-2xl text-base text-muted-foreground sm:text-lg">{c.intro}</p><div className="mt-8 flex flex-wrap gap-3"><Link to={localized("solver/3x3")} className="rounded-lg bg-primary px-5 py-3 text-sm font-medium text-primary-foreground">{c.solve3}</Link><Link to={localized("solver/2x2")} className="rounded-lg border border-border bg-background px-5 py-3 text-sm font-medium">{c.solve2}</Link></div></div><div className="panel flex justify-center overflow-hidden p-4 sm:p-6"><CubeNet state={PREVIEW} /></div></div></section>
