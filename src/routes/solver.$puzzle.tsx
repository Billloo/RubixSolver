        ...
        <section className="panel p-5 sm:p-7">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-lg font-semibold">
              {solution ? "Follow the solution" : "Paint your cube"}
            </h2>
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Hold the cube with <span className="text-foreground">white on top</span> and{" "}
            <span className="text-foreground">green facing you</span>, then click stickers to match
            your cube.
          </p>
                ...
                >
                  ...
                  <span
                    className={cn(
                      ...
                      (counts[c] ?? 0) === n * n ? "text-muted-foreground" : "text-primary",
                    )}
                  >
                    {counts[c] ?? 0}/{n * n}
                  </span>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Button size="lg" onClick={handleSolve} disabled={busy || !!solution}>
                {busy ? (
                  ...
                  <>
                    <Wand2 className="size-4" /> Solve my cube
                  </>
                )}
              </Button>
              {solution && (
          ...
          {solution ? (
            ...
            <div>
              ...
              <ol className="mt-4 space-y-4 text-sm text-muted-foreground">
                <li>
                  <span className="font-medium text-foreground">1. Enter your cube.</span> Pick a
                  color, then click each sticker. Centers are locked so the orientation always
                  matches the instructions.
                </li>
                <li>
                  <span className="font-medium text-foreground">2. Hit solve.</span> The engine
                  {n === 2
                    ? " searches every one of the 3.6 million states for the shortest solution."
                    : " runs Kociemba's two-phase algorithm and returns about 20 moves."}
                </li>
                <li>
                  <span className="font-medium text-foreground">3. Turn along.</span> Step through
                  the moves one at a time — the diagram updates with every turn.
                </li>
              </ol>
              <div className="mt-6 rounded-xl border border-border bg-surface-2 p-4 text-sm text-muted-foreground">
                ...
                Notation: <span className="text-foreground">R</span> = right face clockwise,{" "}
                <span className="text-foreground">R'</span> = counter-clockwise,{" "}
                <span className="text-foreground">R2</span> = half turn. Same for U (up), F
                (front), D (down), L (left), B (back).
              </div>
            </div>
          )}
        </aside>
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-baseline justify-between">
        ...
        <span className="text-sm text-muted-foreground">
          {solution.length} moves · {size}x{size}
        </span>
      </div>
      ...
      <div className="mt-5 flex flex-wrap gap-1.5">
        {solution.map((mv, i) => (
          <button
            ...
            onClick={() => setStep(i)}
            className={cn(
              "min-w-9 rounded-lg border border-border px-2 py-1.5 font-mono text-sm transition hover:border-primary",
              i < step && "text-muted-foreground opacity-60",
